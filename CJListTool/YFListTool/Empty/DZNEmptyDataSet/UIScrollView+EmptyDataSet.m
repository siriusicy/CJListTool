...
//        view.frame = CGRectMake(0, [self emptyBackgroundViewTop], self.frame.size.width, self.frame.size.height);

        view.translatesAutoresizingMaskIntoConstraints = NO;
        NSLayoutConstraint *constraint1 = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1.0 constant:[self emptyBackgroundViewTop]];
        ...
        NSLayoutConstraint *constraint3 = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeWidth multiplier:1.0 constant:0];
        NSLayoutConstraint *constraint4 = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeHeight multiplier:1.0 constant:0];
...
- (CGFloat)emptyBackgroundViewTop {
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(emptyBackgroundViewTopForEmptyDataSet:)]) {
        return [self.emptyDataSetSource emptyBackgroundViewTopForEmptyDataSet:self];
    }
    return 0;
}
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        if ([self canShowButton]) {
            ...
            NSLayoutConstraint *center = [NSLayoutConstraint constraintWithItem:self.contentView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:_button attribute:NSLayoutAttributeCenterX multiplier:1 constant:0];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(self.customView.frame, point)) {
        if ([self.customView respondsToSelector:@selector(hitTest:withEvent:)]) {
            CGPoint pointInCustomView = [self convertPoint:point toView:self.customView];
            return [self.customView hitTest:pointInCustomView withEvent:event];
        }
    }
