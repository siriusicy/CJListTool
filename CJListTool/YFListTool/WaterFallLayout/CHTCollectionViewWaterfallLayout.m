...
- (id)copyWithZone:(NSZone *)zone {
    CHTCollectionViewDecorLayoutAttributes *newAttributes = [super copyWithZone:zone];
    newAttributes.view = self.view;
    return newAttributes;
}

@end

@implementation CHTCollectionDecorReusableView

- (void)applyLayoutAttributes:(UICollectionViewLayoutAttributes *)layoutAttributes {
    [super applyLayoutAttributes:layoutAttributes];
    [self.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    CHTCollectionViewDecorLayoutAttributes *ecLayoutAttributes = (CHTCollectionViewDecorLayoutAttributes*)layoutAttributes;
    [self addSubview:ecLayoutAttributes.view];
}
...
- (id <CHTCollectionViewDelegateWaterfallLayout> )delegate {
    id <CHTCollectionViewDelegateWaterfallLayout> d = (id <CHTCollectionViewDelegateWaterfallLayout> )self.collectionView.delegate;
    if ([d isKindOfClass:NSClassFromString(@"_RXDelegateProxy")]) {
        NSObject *proxy = d;
        return (id <CHTCollectionViewDelegateWaterfallLayout> )[proxy valueForKey:@"_forwardToDelegate"];
    } else {
        return d;
    }
}
...
- (void)prepareLayout {
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        for (idx = 0; idx < itemCount; idx++) {
            ...
            maxY = MAX(yOffset + itemHeight, maxY);
        ...
        if (self.sectionBgType == CHTCollectionViewSectionBgType_View) {
            ...
            decorationAttributes.frame = CGRectMake(0,
                                                    top-sectionInset.top,
                                                    self.collectionViewContentSize.width,
                                                    maxY-top+sectionInset.bottom+sectionInset.top);
            if ([self.delegate respondsToSelector:@selector(collectionView:layout:backgroundViewForSectionAt:)]) {
                decorationAttributes.view = [self.delegate collectionView:self.collectionView layout:self backgroundViewForSectionAt:section];
            }
            decorationAttributes.zIndex = attributes.zIndex-1;
            [self.allItemAttributes addObject:decorationAttributes];
        }
